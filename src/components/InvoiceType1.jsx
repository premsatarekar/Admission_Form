...
const InvoiceType1 = ({ row, onClose }) => {
  ...
  const feesPaid = parseFloat(row.feesPaid) || 0;
  ...
  const handleGeneratePDF = () => {
    ...
    const opt = {
      margin: 10,
      filename: `Invoice_${row.name || 'Student'}_${new Date()
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-4">
        ...
        <p className="text-muted mb-0">Invoice for Course Enrollment</p>
      </div>
        ...
        <div className="col-md-6">
          <div className="card border-0">
            <div className="card-body p-2">
              ...
              <p className="mb-1">
                Invoice #: {row.invoiceNumber || 'VZL-00000'}
              </p>
              <p className="mb-1">
                Date:{' '}
                {row.date ||
      ...
      <div className="table-responsive mb-4">
        <table className="table table-bordered">
          ...
          <tbody>
            <tr>
              <td>Course Fee</td>
              <td className="text-end">{formatINR(courseFee)}</td>
              <td className="text-end fw-bold">{formatINR(courseFee)}</td>
            </tr>
            <tr>
              <td>Discount ({row.discount || 0}%)</td>
              <td className="text-end">-{formatINR(discountAmount)}</td>
              <td className="text-end fw-bold">-{formatINR(discountAmount)}</td>
            </tr>
            <tr>
              <td>Net Amount</td>
              <td className="text-end">{formatINR(netAmount)}</td>
              <td className="text-end fw-bold">{formatINR(netAmount)}</td>
            </tr>
            <tr>
              <td>SGST (9%)</td>
              <td className="text-end">{formatINR(sgst)}</td>
              <td className="text-end fw-bold">{formatINR(sgst)}</td>
            </tr>
            <tr>
              <td>CGST (9%)</td>
              <td className="text-end">{formatINR(cgst)}</td>
              <td className="text-end fw-bold">{formatINR(cgst)}</td>
            </tr>
      ...
      <div className="bg-warning bg-opacity-10 p-3 mb-3 rounded">
        <p className="mb-0 text-center fw-bold">
          Thank you for enrolling with VIZIONEXL TECHNOLOGIES!
        </p>
      </div>
      ...
      <div className="card border-0 mb-4">
        <div className="card-body p-3">
          ...
          <ol className="mb-0" style={{ fontSize: '0.9rem' }}>
            <li>All fees paid are non-refundable once payment is made.</li>
            <li>
              Course access will be provided only after full payment is
              received.
            </li>
            <li>Any bank charges must be borne by the student/client.</li>
            <li>Certificates are issued only after meeting requirements.</li>
            <li>Company may change schedule/content without notice.</li>
            <li>Not responsible for delays caused by technical issues.</li>
          </ol>
        </div>
      </div>
      ...
      <div className="text-center">
        <button className="btn btn-primary" onClick={handleGeneratePDF}>
          <FaDownload className="me-2" /> Generate PDF
        </button>
      </div>
